...
earmarks_clean <- earmarks_clean %>%
  mutate(
    clean_recipient = str_to_upper(recipient),
    recipient_category = case_when(
      clean_recipient %in% valid_geo ~ "Government/Municipality",
      grepl("CITY|COUNTY|TOWN|VILLAGE|BOROUGH|MUNICIPAL|DEPT|DEPARTMENT|POLICE|FIRE|SHERIFF|PUBLIC SCHOOL|DISTRICT|STATE|AUTHORITY|COMMISSION|GOVERNMENT|ARMY|RECLAMATION|TRIBE|USDA|NATION|PARISH|MARYLAND TRANSIT|GOVERNOR|GENERAL SERVICES ADMINISTRATION|PORT OF", clean_recipient) ~ "Government/Municipality",
      grepl("UNIVERSITY|COLLEGE|FOUNDATION|SOCIETY|ASSOCIATION|ALLIANCE|INSTITUTE|CENTER|COUNCIL|HOSPITAL|HEALTH|CLINIC|MUSEUM|CHARITY|INC|CORP|LIMITED|NONPROFIT|TRUST|ACADEMY|CLUB|YMCA|YWCA|SCHOOLS|GOODWILL|BIG BROTHER|UNITED WAY|LAND BANK", clean_recipient) ~ "Nonprofit/Other",
      TRUE ~ "Unclassified")
  )
...
sponsor_choices <- c(earmarks_clean$requested_by, earmarks_clean$requested_by_2) %>%
  ...
  na.omit() %>%       # Remove NAs
  sort()
recipient_choices <- c("All", sort(unique(earmarks_clean$recipient_category)))
```
...
selectInput("recipient_sel", "Recipient Type", 
            choices = c("All", "Government/Municipality", "Nonprofit/Other", "Unclassified"))
...
earmarks_f <- reactive({
  ...
  if (input$recipient_sel != "All")
      df <- df %>% filter(recipient_category == input$recipient_sel)
